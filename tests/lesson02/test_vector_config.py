...
class TestVectorConfigsValid:
    ...
    def test_config_validates(self, config_file):
        ...
        assert exit_code == 0, (
            f"Ошибка валидации {config_file}: {stderr}"
        )
...
class TestPostgreSQLTables:
    ...
    @pytest.mark.parametrize("table_name,required_fields", [
        ("auth_events", ["timestamp", "event_type", "source_ip", "severity"]),
        ("nginx_logs", ["timestamp", "source_ip", "method", "path", "status", "severity"]),
        ("dns_queries", ["timestamp", "source_ip", "query_domain", "query_type", "severity"]),
        ("firewall_events", ["timestamp", "source_ip", "dest_ip", "action", "severity"]),
    ])
    def test_table_has_required_fields(self, conn, table_name, required_fields):
        ...
        with conn.cursor() as cur:
            ...
            assert not missing, (
                f"В таблице {table_name} отсутствуют поля: {missing}"
            )
...
class TestDataInPostgreSQL:
    ...
    @pytest.mark.parametrize("table_name,field_name", [

...
> Пятница, 15 марта, 09:00. Конференц-зал FinanceFlow заполнен. За столом — CISO Марина, тимлид Денис, DevOps-инженер Сергей и Алексей.
>
> Марина открыла презентацию. "Коллеги, вчера вечером мы обнаружили подозрительную активность. Кто-то пытался получить доступ к нашим системам. Нужно понять: что произошло, когда началось, и что было скомпрометировано."
>
> Денис кивнул. "У нас есть логи за последние 14 дней. Все события в PostgreSQL, но как найти иголку в стоге сена? Нужно написать запросы, которые найдут все подозрительные активности."
>
> Алексей открыл ноутбук. "Значит, нам нужно найти этапы атаки: разведку, подбор паролей, компрометацию, перемещение по сети. SQL поможет нам построить картину инцидента."
>
> Марина показала на экран. "У нас есть данные из четырёх источников: auth_events, nginx_logs, dns_queries, firewall_events. Нужно найти связи между ними, построить timeline и понять, что украли злоумышленники."
>
> "Начнём с поиска индикаторов компрометации", — сказал Алексей, открывая psql. — "SQL — это наш язык расследований."
...
__Основной инструмент:__ DuckDB с готовыми Parquet-файлами из `data/lite/` (можно использовать PostgreSQL).

__Период расследования:__ Дни 61-74 (14 дней атаки). Все запросы должны покрывать этот период.
...
После обнаружения всех этапов атаки в следующем блоке мы:
- Используем Python для углубленного анализа (расчёт энтропии DNS-доменов, ML для поиска аномалий)
- Визуализируем результаты в Grafana
- Создадим дашборды для мониторинга подобных инцидентов
...
- [ATTACK_PATTERNS.md](./ATTACK_PATTERNS.md) — справочник паттернов атак (без конкретных IoC)
